			...
			setTimeout(function(){
				...
				_that.$http.post(_that.$api + "/wx/event/user_event/list/", formData, {
					...
					.then(function(response) {
						if(response.data.member_type.MemberMini == false) {
							alert("当前没有活动")
						}else{
						...
						if(response.data.event[0].length==1){
								sessionStorage.setItem("operable", JSON.stringify(response.data.event[0][0])); //保存当前点击菜单
								_that.$router.push({
									path: "/user-content"
								})
						}
