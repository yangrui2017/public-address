<template>
	...
	<div class="box">
		...
		<el-row>
			<el-col :span="19">
				<el-form>
					...
					<el-form-item required class="housing_input">
						<el-input id="suggestId" v-model="city" placeholder="请输入地址" name="address_detail" />
						<div id="allmap"></div>
					</el-form-item>
				</el-form>
			</el-col>
		...
		<el-button type="primary" @click="location()" class="zidong">自动定位</el-button>
	...
	export default {
		...
		methods: {
			...
			addresstranslation(res) {
				var _that = this;
				jsonp('http://api.map.baidu.com/geocoder/v2/?address=' + res + '&output=json&ak=DQmr8taZg7UEk3PQe96FG3hqEc6Rihqp&callback=showLocation', null, (err, data) => {
					if(err) {
			...
			https() {
				...
				_that.$http.post("http://dev.upctech.com.cn/api/map/judge_active_area", {
					...
					.then(function(response) {
						if(response.data.active) {
							 _that.$alert('您所查询的地址已开放E帮小管家上门服务，','消息提示',  {
									confirmButtonText: '确定',
									callback: action => {
									}
							});
						} else {
							 _that.$alert('您所查询的地址尚未开放E帮小管家上门服务，敬请谅解。详情请咨询：4008219308','消息提示',  {
									confirmButtonText: '确定',
									callback: action => {
									}
							});
